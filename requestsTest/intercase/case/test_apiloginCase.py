...
from requestsTest import intercase
from requestsTest.intercase.api.baseApi import baseApi
from requestsTest.intercase.tools.getLogger import getLogger
...
class apiloginCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        getLogger().getlogFileFormater(logger=cls.log, level=logging.INFO,
                                       fileName=intercase.filePath.format("apiloginCase"), frm=intercase.frm)
        cls.baseApi=baseApi()
        cls.token=None
        cls.orderIds = None
        # global Token
...
# # print("header:"+"data:"+json.dumps(data))  #字典转json json.loads(data))  #json转字典
    ...
    def setMytoken(cls, token):
        cls.token=token
    @file_data(intercase.IFloginymlDataFileLoc)
    @unpack
    ...
    @file_data(intercase.IFsearchymlDataFileLoc)
    @unpack
    def test_2search(self, **kwargs):
        ...
        try:
            ...
            self.log.info(
                desc + "响应码+success断言失败 " + "code是(实际，期望)：" + re.json()['code'] + "," + exptValueCode)  # 不能是布尔型
    @file_data(intercase.IFprodetailymlDataFileLoc)
    @unpack
    def test_3proDetails(self, **kwargs):
        ...
        try:
            ...
            self.log.info(
                desc + "响应码+success断言失败 " + "code是(实际，期望)：" + re.json()['code'] + "," + exptValueCode)  # 不能是布尔型
    @file_data(intercase.IFaddCartNumymlDataLoc)
    @unpack
