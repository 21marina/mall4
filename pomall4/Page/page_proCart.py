...
class pageProCart(base):
    ...
    def page_proCart(self):
        ...
        self.base_find_element(pomall4.loc_all_btn).click()
        sleep(1)
        # self.base_click(mall4.loc_all_btn)#全选
        self.base_click(pomall4.loc_all_btn)#取消全选
        #找到复选框 判断有无被选 若有选 不选 如未选 点击
        print("挑选商品开始 取消全选 即将选美容仪")
        if self.base_eleIsExit(pomall4.loc_pro_carted):#已选
            pass
        else:
            self.base_click(pomall4.loc_pro_cart)
        print("挑选商品 选择完商品")
        sleep(1)
        # self.driver.execute_script("arguments[0].scrollIntoView();", self.base_find_element(mall4.loc_all_btn))
...
#组装
    def zuzhuangCartPage(self):
        ...
        print("即将去结算")
        sleep(1)
        self.page_proClickjs(pomall4.loc_submit_btn)
        sleep(2)
        # self.page_proOrder()
