...
#选商品要加进来吗？
    ...
    def page_proType(self):
        ...
        if base(self.driver).base_eleIsExit(pomall4.loc_proType):
             self.base_find_element(pomall4.loc_proType).click()

    #加数量 一个一个加
    def page_proCountincr(self,num): #num指的是输入框的值 还是点击数 选点击数
        # num 取绝对值遍历  若是负数 绝对值与num不想等
        for n in range(1,num):
            self.base_find_element(pomall4.loc_num_incr).click()
    def page_proCountdesc(self,num):
        # num 取绝对值遍历  若是负数 绝对值与num不想等
        for n in range(num,0):
            self.base_find_element(pomall4.loc_num_desc).click()
    def page_proCountinput(self, loc,num):
        self.base_input(loc,content=num)
    # 加购
    def page_proCart(self):
        self.base_find_element(pomall4.loc_addCart).click()

#立即购买
    def page_proBuy(self):
        self.base_find_element(pomall4.loc_buy).click()
        #订单
