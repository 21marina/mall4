...
import pomall4
from pomall4.Page.page_index import pageIndex
from pomall4.Page.page_login import pageLogin
from pomall4.Page.page_proCart import pageProCart
from pomall4.Page.page_proDetails import pageProDetails
from pomall4.Base.base import base
from pomall4.Page.page_proOrder import pageOrder
from pomall4.tools.getLogger import getLogger
from pomall4.tools.get_driver import getDriver
import urllib
...
class test02y(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        getLogger().getlogFileFormater(logger=cls.log, level=logging.INFO,
                                       fileName=pomall4.filePath.format("test02y"), frm=pomall4.frm)
...
#     @unittest.skip("search skip")
    @file_data(pomall4.searchymlDataFileLoc)
    @unpack
    def test02_search(self, **kwargs):
        ...
        if base(self.driver).base_eleIsExit(pomall4.loc_msgbox_top):#元素一直存在吗 第二个用例页进来
            print("aa"+base(self.driver).base_find_element(pomall4.loc_msgbox_top).text)
            self.opResult(pomall4.loc_msgbox_top, exceptedValue)
        else:
            print("else 语句")
            sleep(1)
    ...
    @file_data(pomall4.addCartNumymlDataLoc)
    @unpack
    def test02_addCart(self, **kwargs): #显示查询结果列表下的页数切换
        numContnet = kwargs.get("content")
        exceptedValue = kwargs.get("result")
        if numContnet=="stock":
            if base(self.driver).base_eleIsExit(pomall4.loc_num_stock):
                stock=base(self.driver).base_find_element(pomall4.loc_num_stock).text
        ...
        base(self.driver).base_find_element(pomall4.loc_pro).click() #找到商品进商品详情页
        self.pageProDetails.zuzhuangCart(pomall4.loc_num_input,numContnet)
        #v=v.replace(/[^\d]/g,'')
        ...
        if base(self.driver).base_eleIsExit(pomall4.loc_msgaddCart):
            sleep(1)
            self.opResult(pomall4.loc_msgaddCart,exceptedValue) #断言包含其中
        else:
...
#     @unittest.skip("跳过test02_Buy")
    ...
    # 获取商品元素 进入商品详情 选规格 加数量  # 立即购买 # 加断言
        base(self.driver).base_find_element(pomall4.loc_pro).click()
        if numContent == "stock":
            if base(self.driver).base_eleIsExit(pomall4.loc_num_stock):
                stock=base(self.driver).base_find_element(pomall4.loc_num_stock).text
        ...
        self.pageProDetails.zuzhuangBuy(pomall4.loc_num_input, numContent)  # 立即购买之后跳转订单页 怎么断言
        if base(self.driver).base_eleIsExit(pomall4.loc_msgaddCart):  # 没存在 说明点击立即购买没有成功  断言
            sleep(1)
            self.opResult(pomall4.loc_msgaddCart, exceptValue)
            self.log.info("Buybyprod 商品-立即购买失败 但断言成功 提示请输入正确的商品数量 预期也是")
        elif base(self.driver).base_eleIsExit(pomall4.loc_detail_num):
            sleep(1)
            if self.isfushu(numContent):#true 说明是整数或负数  字符串进不来
                self.opResult(pomall4.loc_detail_num, str(abs(int(numContent))))
...
#     @unittest.skip("暂跳过")
    def test02_collect(self):
        base(self.driver).base_find_element(pomall4.loc_pro).click()
        if base(self.driver).base_eleIsExit(pomall4.loc_collected_btn):
            sleep(1)
            value= base(self.driver).base_find_element(pomall4.loc_collected_btn).text
            print("yishoucang :"+value)
        elif base(self.driver).base_eleIsExit(pomall4.loc_collect_btn):# 无论collect是否active 都只会捕捉到collect 不能区分收藏状态
            sleep(1)
            value= base(self.driver).base_find_element(pomall4.loc_collect_btn).text
        ...
        pageProDetails(self.driver).zuzhuangCollect(pomall4.loc_collect_btn)
        #先获取提示信息 再断言 若无提示 记录日志
        if base(self.driver).base_eleIsExit(pomall4.loc_msgaddCollect):
            self.opResult(pomall4.loc_msgaddCollect, exceptValue)
        else:
...
#引用带文件的方法 会读取到其文件吗
    def test02_order1(self, **kwargs):
        # 点击购物车图标 购物车结算生成订单
        self.pageProCart.page_proClick(pomall4.loc_cart_top)
        sleep(1)
    ...
    @file_data(pomall4.jifenymlDataLoc)
    @unpack
    ...
    # 立即购买生成订单     # 选商品 点击结算 下订单（地址+折扣+商品详情）
        ...
        #立即购买
        base(self.driver).base_find_element(pomall4.loc_buy).click()
        self.pageOrder.zuzhuangOrder(pomall4.loc_jifendisc_input,numContent)
    #断言
    #传的空 字符 中文 特殊字符 实际值￥0.00  #传超过最大值 实际值￥0.00 #传负数 实际值绝对值
        if numContent=="stock":
            if base(self.driver).base_eleIsExit(pomall4.loc_jifendisc_max):
                #获取当前商品订单可抵扣的最大值 大于此值时会是此值
                numContent = base(self.driver).base_find_element(pomall4.loc_jifendisc_max).text #（该订单最多可用 9 积分）
                numContent= list(filter(str.isdigit,numContent))
                numContent=str(int(numContent[0]) + 1) #输入比当前可抵扣数额大一的值
            elif base(self.driver).base_eleIsExit(pomall4.loc_jifendisc_maxAccount):#还有一个账户最大可用值
                numContent = base(self.driver).base_find_element(pomall4.loc_jifendisc_maxAccount).text#账户共 2001 积分，本次使用
                numContent = list(filter(str.isdigit, numContent))
        ...
        if self.isfushu(numContent):  # true 说明是整数或负数  字符串进不来
            if exptValue is None: # 超过可抵扣最大值 断言 实际值为max
                ...
                self.opResult(pomall4.loc_jifendisc_disc, '￥0.00')
            else:  # stock的预期值为空 真实值会变成最大值
                self.opResult(pomall4.loc_jifendisc_disc,'￥'+str(abs(int(numContent)))+'.00')
        else:
            self.opResult(pomall4.loc_jifendisc_disc, exptValue) #预测值和content值差不多

    @file_data(pomall4.caseyewuDataLoc)
    @unpack
    def test_yewu1(self,**kwargs):#切记是双星
        ...
        base(self.driver).base_find_element(pomall4.loc_pro).click() #进入商品详情
        self.log.info("进入商品详情")
        self.pageProDetails.zuzhuangBuy(pomall4.loc_num_input, numContent)#输入数量 立即购买
        self.log.info("输入数量 立即购买")
        self.pageOrder.zuzhuangOrder(pomall4.loc_jifendisc_input, numjfContent)
    ...
    def getLogin(self):
        ...
        pwd="123456"
        base(self.driver).base_find_element(pomall4.loc_denglulink).click()
        pageLogin(self.driver).zuzhuang(userName, pwd, pomall4.loc_username, pomall4.loc_pwd, pomall4.loc_denglusub)
