...
from pomall4.tools.op_yaml import OpYaml


def get_data():
    re=OpYaml().get_yml(pomall4.ymlDataFileLoc)
...
class testLogin01y(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        print("setupendingloginy")
        cls.log = getLogger().get_logger()
        getLogger().getlogFileFormater(logger=cls.log, level=logging.INFO, fileName=pomall4.filePath.format("testLogin01y"), frm=pomall4.frm)
    ...
    @file_data(pomall4.loginymlDataFileLoc)
    @unpack
    def test02_login(self, **kwargs):
        ...
        self.pageLogin.zuzhuang(userName, pwd,pomall4.loc_username,pomall4.loc_pwd,pomall4.loc_denglusub)
        #TypeError: zuzhuang() missing 1 required positional argument: 'self'
        # 要么用类对象 要么用self.类
        if base(self.driver).base_eleIsExit(pomall4.loc_msgbox_top):
            #等于true 说明有元素 元素存在说明头上提示用户名或密码不正确
            self.opResult(pomall4.loc_msgbox_top, exceptedValue)
        elif base(self.driver).base_eleIsExit(pomall4.loc_msgbox_user):
             # 元素不存在 有可能是成功 也有可能提示在输入框下面 比如用户名为空 密码
            self.opResult(pomall4.loc_msgbox_user, exceptedValue)
        elif base(self.driver).base_eleIsExit(pomall4.loc_msgbox_pwd):
            self.opResult(pomall4.loc_msgbox_pwd, exceptedValue)
